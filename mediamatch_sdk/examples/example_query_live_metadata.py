...
def main():
    ...
    try:
        # Option 1: get live metadata with asset_id
        metadata = sdk_client.live_asset_service.get_live_asset_metadata(asset_id=7277926702888583000)
        print("get live metadata with asset_id")
        print(json.dumps(metadata))

        # Option 2: query live metadata page by page
        metadata_list = sdk_client.live_asset_service.query_live_asset_metadata(page=10, pageSize=20)
        print("query live metadata with page and pageSize")
        print(json.dumps(metadata_list))

        # You could also query live metadata with filtered contentCategories and tags
        filtered_metadata_list = sdk_client.live_asset_service.query_live_asset_metadata(
            page=1, pageSize=20,
            contentCategories=[ContentCategory.tv, ContentCategory.sports],
            tags=['entretenimento']
        )
        print("query live metadata with filters page by page")
        print(json.dumps(filtered_metadata_list))

        # Option 3: retrieve all the asset metadata with sdk
        asset_metadata_all = sdk_client.live_asset_service.retrieve_all_asset_metadata()
        ...
        filtered_asset_metadata_all = sdk_client.live_asset_service.retrieve_all_asset_metadata(
            contentCategories=[ContentCategory.tv, ContentCategory.sports],
            tags=['entretenimento']
        )
