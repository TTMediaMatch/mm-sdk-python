...
def main():
    ...
    try:
        # Option 1: get video other_info with asset_id
        asset_other_info = sdk_client.video_asset_service.get_video_asset_other_info(asset_id=7277926702888583000)
        print("get video other_info with asset_id")
        print(json.dumps(asset_other_info))

        # Option 2: query video other_info page by page
        other_info_list = sdk_client.video_asset_service.query_video_asset_other_info(page=10, pageSize=20)
        print("query video other_info with page and pageSize")
        print(json.dumps(other_info_list))

        # query video other_info with filtered contentCategories and tags
        filtered_other_info_list = sdk_client.video_asset_service.query_video_asset_other_info(
            page=1, pageSize=20,
            contentCategories=[ContentCategory.tv, ContentCategory.sports]
        )
        print("query video other_info with filters page by page")
        print(json.dumps(filtered_other_info_list))

        # Option 3: retrieve all the asset other_info with sdk
        asset_other_info_all = sdk_client.video_asset_service.retrieve_all_asset_other_info()
