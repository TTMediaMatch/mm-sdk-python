...
class LiveMatchQuery(BaseClient):
    def __init__(self, access_token):
        super().__init__(access_token)  # Initialize the BaseClient with the access token

    def get_live_match(self, asset_id,start_time,end_time):
        response = self._get(path=f"openapi/asset/v1/live/assets/{asset_id}/streamPushInfo?startTime={start_time}&endTime={end_time}")
